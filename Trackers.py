...
class LogsTracker:
    ...
    def listenUntilExit(self, timeout, responseCall, args=""):
        if not self.hasThread:
            threading.Thread(target=self._listenUEThread,
                             args=(timeout, responseCall, args)).start()
        else:
            self.refresh()

    def _listenUEThread(self, timeout, responseCall, args):
        self.hasThread = True
        try:
            if os.path.isfile(self.logPath):
                self.start = time.time()
                ...
                while self.running and (timeout == -1 or time.time() - self.start < timeout):
                    ...
                    if mtime != newmtime:
                        ...
                        if newLen > lineLen:
                            for line in lines[lineLen: newLen]:
                                if "opping singleplayer server as player logged" in line.lower():
                                    self.hasThread = False
                                    self.running = False
                                    responseCall(*args)
                        lineLen = newLen
